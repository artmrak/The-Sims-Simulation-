...
class Human:
    ...
    def shopping(self, manage):
        if self.car.drive():
            ...
            if self.car.fuel < 20:
                ...
                return
        if manage == "fuel":
            print("I have bought fuel for my car")
            self.money -= 100
            self.car.fuel += 100
        elif manage == "food":
            print("I have bought some food")
            self.money -= 50
            self.home.food += 50
        elif manage == "delicacies":
            print("Wow! It was truly delicious!")
            self.gladness += 10
            self.satiety += 2
            self.money -= 15
    ...
    def live(self, day):
        ...
        if self.car is None:
            self.get_car()
            print(f"I bought a new car {self.car.brand}")
        if self.job is None:
            self.get_job()
            print(f"I don't have a job, looks like I have to get a job {self.job.job} with salary {self.job.salary}")
        self.days_indexes(day)

        dice = random.randint(1, 4)
        if self.satiety < 20:
            print("Damn, I`m hungry, I will go and eat something")
            self.eat()
